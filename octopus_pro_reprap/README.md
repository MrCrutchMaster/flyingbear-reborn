...
Стартовый gcode указывать не нужно, он уже лежит на карте sys/start.g и будет выполняться автоматически

Финишный gcode нужно указать `M0;` тогда будет вызываться финишный код с карты sys/stop.g


## Pressure Advance / Retract

Перед печатью нужно всегда сменять филамент в виджете Tools, даже если уже стоит нужный. Это подгрузит настройки PA и ретракта для этого филамента из его файла config (После перезапуска принтера они слетают на дефолтные).

Чтобы работали ретракты указанные в конфиге филамента, в слайсере необходимо включить откаты, указав любые отличные от 0 значения, а так же включить опцию "Откат встроеного программного обеспечения" (Cura, другие - хз), это заменит все ретракты в gcode на команду G10, по которой будет делаться ретракт с настроками принтака.
